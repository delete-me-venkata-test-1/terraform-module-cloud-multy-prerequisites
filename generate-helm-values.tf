...
module "glueops_platform_helm_values" {
  ...
  loki_aws_access_key                        = aws_iam_access_key.loki_s3[each.value.environment_name].loki.id
  loki_aws_secret_key                        = aws_iam_access_key.loki_s3[each.value.environment_name].loki.secret
